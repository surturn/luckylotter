...
- [ ] Existing rows keep their pre-fix `avg_interval_days` until the customer's next transaction triggers a recompute. Harmless for the seeded demo (its visits are already spaced), but a business that has imported real POS history needs a one-off recompute pass before those thresholds mean anything (FR-2)
