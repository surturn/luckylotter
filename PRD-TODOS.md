...
- [x] ~~**Decided: refined top nav, not a sidebar.**~~ Four destinations don't earn a permanent ~240px column, and the two screens that matter — the flag table and the visit timeline — are the width-hungry ones; a sidebar would spend width on navigation that the content is short of (support)
- [x] **Reversed 2026-08-13: sidebar, on a design direction.** The cost the original decision named is real and is now being paid — the content area is ~240px narrower on desktop. What changed is the judgement, not the arithmetic: a persistent left rail reads as an operations tool rather than a settings page, and it gives the tenant identity a fixed home instead of competing with navigation across the top. The supplied mockups showed a sidebar **and** a top nav carrying the same four destinations; the duplicate was not implemented, since two copies of the same navigation double where a user has to look and take back the vertical space the sidebar just spent. The top strip keeps only what is about the session — which business is on screen, and sign out (support)
...
- [x] **Decided 2026-08-13: a backdated transaction does not resolve an open flag.** Only a transaction that moves `last_visit_at` forward answers a flag — deliberately the same condition that already governed the forward-only last-visit rule, so a transaction that cannot move the last-visit date cannot close the flag either. Without this, importing a year of POS history for a currently-lapsed customer would close their flag on a months-old receipt, reporting them as recovered when nobody has seen them, silently and across a whole file at once. A backdated sale is still real history: it counts toward `transaction_count` and the cadence, it just doesn't answer the flag. `IngestionBackdatingTest` pins all three directions — backdated leaves it open, live still resolves, and the sale still counts (FR-9, FR-1)
